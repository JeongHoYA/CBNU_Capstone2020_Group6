...
public class Clouds : MonoBehaviour
{
    ...
    bool[,] cloudData; // Arrayy of bools representing where cloud is;
        ...
        for (int xIncrement = 0; xIncrement < cloudTileSize; xIncrement++)
        {
            for (int zIncrement = 0; zIncrement < cloudTileSize; zIncrement++)
            {
                ...
                if (cloudData[xVal, zVal])
                {
                    ...
                    for (int p = 0; p < 6; p++)
                    {
                        // If the current neighbour has no clouds, draw this face.
    ...
    {
        // Because clouds are 2D, if y is above or below 0, returen false.
        if (point.y != 0)
            return false;
    ...
    private int CloudtileCoordFromFloat (float value)
    {
        ...
        a -= Mathf.FloorToInt(a); // Subtract whole number to get a 0-1 value representing position in cloud texture
