...
public class MainMenu : MonoBehaviour
{
    ...
    public BuildSettings buildSettings;                                                                             // 세팅 클래스
    ...
    {
        buildSettings.mapName = mapNameField.text;
        nameFromMainScene = buildSettings.mapName;

        
        if(buildSettings.mapName == null)
        {
            notValidNamePanel.SetActive(true);
        } 
        else if(Directory.Exists(Application.dataPath + "/MapFolder/" + buildSettings.mapName + "/"))
        {
            notValidNamePanel.SetActive(true);
        ...
        {
            Directory.CreateDirectory(Application.dataPath + "/MapFolder/" + buildSettings.mapName + "/");
            locationFromMainScene = Application.dataPath + "/MapFolder/" + buildSettings.mapName + "/";

            string jsonExport = JsonUtility.ToJson(buildSettings);
            File.WriteAllText(locationFromMainScene + buildSettings.mapName + " settings.cfg", jsonExport);
            ...
            {
                mapListFile.WriteLine(buildSettings.mapName);
                savedMapList.Add(buildSettings.mapName);
            }
    ...
    {
        buildSettings.mapName = nameFromMainScene;
        locationFromMainScene = Application.dataPath + "/MapFolder/" + buildSettings.mapName + "/";
        Invoke("StartBuildScene", 3f);
        // StartBuildScene 3초 후에 실행"
    }                                                                                   // 저장된 빌드 맵 로드 함수
    ...
    public void DeleteBuildMap()
    {
        ...
        foreach(string l in savedMapList)
        {
            ...
            {
                Debug.Log("delete " + savedMapList[cnt] + " = " + l);
...
public class SavedMap
{
    ...
    public bool isMapCanBeCleared = false;
}
