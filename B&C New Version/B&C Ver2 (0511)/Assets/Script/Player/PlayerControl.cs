...
public class PlayerControl : MonoBehaviour
{
    ...
    public Text selectedBlockText;                                                              // 선택된 블록의 텍스트
    public byte selectedBlockIndex = 1;                                                         // 선택된 블록의 인덱스
    ...
    private void Start()
    {
        ...
        selectedBlockText.text = world.blocks[selectedBlockIndex].blockName + " 선택됨";
    }
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0)) {
            ...
            {
                // 오브젝트 생성
                Instantiate(world.blocks[selectedBlockIndex].block, instantiatePos, Quaternion.identity, world.GetChunkFromVector3(instantiatePos).transform);
                // 복셀맵 리스트 값 변경
                world.GetChunkFromVector3(instantiatePos).EditVoxelMap(instantiatePos, selectedBlockIndex);
            }
        }// 오브젝트 생성 및 제거 기능  
    }
    ...
    private void PlayerMovement()
    {
        ...
        if(scroll != 0)
        {
            ...
            if (selectedBlockIndex > (byte)(world.blocks.Length - 1))
                selectedBlockIndex = 1;
            if (selectedBlockIndex < 1)
                selectedBlockIndex = (byte)(world.blocks.Length - 1);
