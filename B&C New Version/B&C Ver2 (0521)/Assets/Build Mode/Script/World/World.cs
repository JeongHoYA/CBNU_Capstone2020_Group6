...
public class World : MonoBehaviour
{
    ...
    public BuildModeSettings buildSettings;                                                 // 세팅 클래스
    ...
    void ImportSetting()
    {
        ...
        buildSettings = JsonUtility.FromJson<BuildModeSettings>(jsonImport);
    }                                                                 // 변경된 설정을 Json파일로 Import해주는 함수

    public void SetGlobalLightValue()
    {
        if(buildSettings.isDay)
        {
    ...
    {
        for (int x = playerLastChunkCoord.x - buildSettings.viewDistance; x < playerLastChunkCoord.x + buildSettings.viewDistance; x++)
        {
            for (int z = playerLastChunkCoord.z - buildSettings.viewDistance; z < playerLastChunkCoord.z + buildSettings.viewDistance; z++)
            {
    ...
    {
        for (int x = playerLastChunkCoord.x - buildSettings.viewDistance - 1; x < playerLastChunkCoord.x + buildSettings.viewDistance + 1; x++)
        {
            for (int z = playerLastChunkCoord.z - buildSettings.viewDistance - 1; z < playerLastChunkCoord.z + buildSettings.viewDistance + 1; z++)
            {
                if (IsChunkInWorld(x, z))
                {
    ...
    void CheckViewDistance()
    {
        ...
        for (int x = coord.x - buildSettings.viewDistance; x < coord.x + buildSettings.viewDistance; x++)
        {
            for (int z = coord.z - buildSettings.viewDistance; z < coord.z + buildSettings.viewDistance; z++)
            {
...
[System.Serializable]
public class BuildModeSettings
{
    public string mapName = "";                                                             // 맵 이름
